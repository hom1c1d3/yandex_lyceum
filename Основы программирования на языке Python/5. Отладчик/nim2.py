...
from operator import xor  # xor(n, m) тоже самое, что n^m
...
def main():
    ...
    for i in range(num_piles):
        rocks = int(input(f"Введите количествот камней для {i + 1} кучки: "))
        if rocks > 0:
            piles.append(rocks)
        else:
            raise TypeError("Количество камней не может буть меньше одного")
    print()
    ...
    while any(piles):
        ...
        print("Из кучки", pile_num + 1)
        print("ИИ взял", computer_move)
        print("Итого:\n", '\n'.join(
            (f'В {i + 1} кучке: {p} камней ' for i, p in enumerate(map(str, piles)))))
        if not any(piles):
            print("ИИ выиграл!")
            break
        pile_num = int(input("Введите номер кучки, из которой вы будете брать: "))
        true_piles_ind = (ind for ind, value in enumerate(piles))
        while pile_num - 1 not in true_piles_ind:
            print(f'Кучки номер {pile_num} не существует')
            pile_num = int(input("Введите номер кучки, из которой вы будете брать: "))
            true_piles_ind = (ind for ind, value in enumerate(piles))
        user_move = int(input("Сколько камней вы возьмете: "))
        while user_move > piles[pile_num - 1] or user_move < 1:
            print(f'Вы не можете взять {user_move} камней из {pile_num} кучки')
            user_move = int(input("Сколько камней вы возьмете: "))
        piles[pile_num - 1] -= user_move
        print("Из кучки", pile_num)
        print("Вы взяли", user_move)
        print("Итого:", '\n'.join(
            (f'В {i + 1} кучке: {p} камней ' for i, p in enumerate(map(str, piles)))))
        if not any(piles):
            print('Вы выиграли!')
