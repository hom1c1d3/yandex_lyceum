...
class MainWindow(QtWidgets.QMainWindow, Ui_Form):
    ...
    def init_widgets(self):
        for i in (getattr(self, i) for i in self.__dict__ if i[-1].isnumeric()):
            i.clicked.connect(self.add_to_display)
        self.btn_dot.clicked.connect(self.add_to_display)
        for i in (getattr(self, i) for i in self.__dict__ if
                  i.startswith('btn') and not i[-1].isnumeric() and i not in (
                  'btn_clear', 'btn_eq', 'btn_dot')):
            i.clicked.connect(self.add_sign)
        self.btn_eq.clicked.connect(self.solve)
        self.btn_clear.clicked.connect(self.clear)

    def clear(self):
        self.current_expression = ['']
        self.table.display(self.current_expression[0])

    def add_to_display(self):
        self.current_expression[-1] += self.sender().text()
        self.table.display(self.current_expression[-1])

    def solve(self):
        for ind, i in enumerate(self.current_expression):
            if i == '^':
                self.current_expression[ind] = '**'
            elif i == '√':
                math.pi.hex()
                self.current_expression[ind] = 'math.sqrt('
                self.current_expression[ind + 1] = self.current_expression[ind + 1] + ')'
            elif i == '!':
                self.current_expression[ind] = ')'
                self.current_expression[
                    ind - 1] = f'math.factorial({self.current_expression[ind - 1]}'
        try:
            res = str(eval(''.join(self.current_expression)))[:5]
            self.current_expression = [res]
        except (ValueError, SyntaxError, ZeroDivisionError):
            res = 'Error'
            self.current_expression = ['']
        self.table.display(str(res))

    def add_sign(self):
        self.current_expression.append(self.sender().text())
